...
app.command("/johnpork-ping", async ({ command, ack, respond }) => {
  ...
  await respond({ text: `Hope I porked here fast enough!\nLatency: ${latency}ms` });
});
...
app.command("/johnpork-catfact", async ({ ack, respond }) => {
  ...
  try {
    ...
    await respond({ text: `John Pork: Cats are weird!\n${response.data.fact}` });
  } catch (err) {
...
app.command("/johnpork-randomquranayah", async ({ ack, respond }) => {
  // 1. Immediately acknowledge to beat Slack's 3-second limit
  await ack();

  // 2. Pick a random ayah number (1 to 6236)
  const randomAyahNumber = Math.floor(Math.random() * 6236) + 1;

  try {
    // Specify the edition (e.g., 'en.asad' for English, or leave off /en.asad for Arabic)
    ...
    console.error(error);
    await respond({ text: "John Pork couldn't fetch a verse right now. Try again later!" });
  }
});
      ...
      blocks: [
        {
          ...
          text: {
            type: "mrkdwn",
            text: "*John Pork:* Here's some food for you!"
          }
        },
        {
          type: "image",
          image_url: imageUrl,
          alt_text: "Random delicious food"
        }
      ]
    });
  } catch (err) {
    console.error(err);
    await respond({ text: "Failed to fetch a food image." });
  }
});
...
app.command("/johnpork-call", async ({ ack, respond }) => {
  ...
  try {
    ...
    console.error(err);
    await respond({ text: "John Pork declined the call." });
  }
});
...
app.command("/johnpork-weather", async ({ command, ack, respond }) => {
  ...
  // If the user doesn't provide a city, pick a default
